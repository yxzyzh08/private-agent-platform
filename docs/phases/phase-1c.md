...
**目标**: Owner 在 cui 中描述新需求 → Claude Code 交互澄清 → LLM 分解为 N 个独立子任务 → dev_bot 逐个调用 Claude CLI 执行（每个子任务=全新上下文）→ 验证 → 创建 PR。解决 Claude CLI 单次会话上下文膨胀导致质量下降的问题。
**预计时长**: 2 周

**完成条件**: Owner 在 cui 中描述一个需求后，系统自动分解为子任务列表 → Owner 确认 → 子任务逐个独立执行（每个任务全新 Claude CLI 上下文）→ 每个子任务完成后自动 commit checkpoint → 全部完成后创建 PR → ntfy 通知 Owner
...
| **需求开发模式** | Owner 在 cui 中描述需求 | LLM 分解 → 多次独立 Claude CLI 调用 | 1C（新增） |
...
```
┌─────────────────────────────────────────────────────────┐
│ 阶段 1: 需求澄清（Owner ↔ Claude Code 在 cui 中对话）      │
│                                                         │
...
└───────────────────────┬─────────────────────────────────┘
                        ↓
┌───────────────────────▼─────────────────────────────────┐
│ 阶段 2: 任务分解（LLM 自动，Owner 审批）                   │
│                                                         │
│  LLM 分解为 5 个子任务，Owner 在 cui 中审查/调整/确认       │
│  确认后持久化到 data/agents/dev_bot/workspace/task_plans/ │
└───────────────────────┬─────────────────────────────────┘
                        ↓
┌───────────────────────▼─────────────────────────────────┐
│ 阶段 3: 多任务串行执行（自动，后台运行）                    │
│                                                         │
...
### Task 1C.2: 实现 core/task_planner.py — LLM 任务分解引擎
...
**描述**:
使用 LLM（通过 AgentRuntime + LiteLLM）将澄清后的需求文本分解为结构化子任务列表。

**分解函数接口**:
```python
async def decompose_requirement(
    requirement: str,
    project_context: str,      # CLAUDE.md 摘要 + 目录结构
    runtime: AgentRuntime,
) -> list[SubTask]:
```

**LLM Prompt 策略**:
- 输入：需求文本 + 项目目录结构 + CLAUDE.md 关键规则摘要
- 输出：JSON 数组，每个元素包含 title、description、depends_on、context_files、validation_command
- 约束：JSON Schema 验证输出格式；子任务数不超过 `max_subtasks` 配置值（默认 10）
- 每个子任务描述中自动注入："不要修改 .env、credentials 等敏感文件"

**验收标准**:
- [ ] `decompose_requirement()` 调用 LLM 返回 `list[SubTask]`
- [ ] LLM 输出经 JSON Schema 校验，格式错误时重试一次
- [ ] 子任务数量超过 `max_subtasks` 时截断并在日志中记录 WARNING
- [ ] 每个子任务的 description 包含敏感文件保护指令
- [ ] `depends_on` 中引用的 task_id 必须存在，否则校验失败
- [ ] 项目上下文通过读取目录结构和 CLAUDE.md 自动生成（`_build_project_context()`）

**测试命令**:
```bash
uv run pytest tests/unit/test_task_planner.py -v -k "test_decompose"
```

**测试用例**:
- test_decompose_simple_requirement — 简单需求分解为 2-3 个任务
- test_decompose_complex_requirement — 复杂需求分解为 5+ 个任务（含依赖）
- test_decompose_max_subtasks_limit — 超出上限时截断
- test_decompose_invalid_llm_output — LLM 输出格式错误时重试
- test_decompose_invalid_depends_on — 引用不存在的 task_id 时校验失败
- test_build_project_context — 项目上下文构建
...
class DevAgent(BaseAgent):
    ...
    async def handle_requirement(self, requirement: str, source: str = "cui") -> TaskPlan:
        """需求开发模式入口：分解 → 返回 draft plan 等待确认"""

    async def approve_plan(self, plan_id: str) -> TaskPlan:
        """Owner 确认后开始执行"""
...
**验收标准**:
- [ ] `handle_requirement()` 调用 `decompose_requirement()`，创建 draft TaskPlan，发送 ntfy 通知
- [ ] `approve_plan()` 将 plan.status 设为 approved，创建 feature 分支，启动 `execute_plan()`
...
**测试用例**:
- test_handle_requirement — 需求分解 + draft plan 创建
- test_approve_plan — 确认后创建分支并执行
...
**新增端点**:
```
POST   /api/requirements          → 提交新需求（触发分解）
GET    /api/requirements/{id}     → 查询 plan 状态和进度
POST   /api/requirements/{id}/approve    → 确认执行
...
**测试用例**:
- test_create_requirement — POST 创建需求返回 plan
- test_get_plan_status — GET 返回进度
- test_approve_plan — POST approve 触发执行
