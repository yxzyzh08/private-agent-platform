...
4. **Smart Checkpoint**: 检查 `git status --porcelain`，只在有未提交变更时：
   - `git add -A`
   - **敏感文件预检**: `git diff --cached --name-only` 对比 `sensitive_patterns`，命中则 `git reset HEAD -- <file>` 移出暂存区 + 通知 Owner
   - `git commit`
5. **敏感文件全量检测**: `git diff --name-only {checkpoint_sha} HEAD` 检查从起点到当前所有 commit 的变更，命中则 `git reset --hard {checkpoint_sha}` 回滚到子任务执行前的状态
6. **回写 Markdown**: 调用 `update_task_status(phase_file, task_id, "x")`
7. **持久化 JSON**: 更新 TaskPlan 并 save
8. **失败后**: `git reset --hard {checkpoint_sha}` 回滚到子任务执行前状态
...
**描述**:
两阶段敏感文件保护：
1. **预提交检查**（Smart Checkpoint 中）：`git add -A` 后、`commit` 前，用 `git diff --cached --name-only` 检查暂存区，命中敏感模式则 `git reset HEAD -- <file>` 移出暂存区
2. **全量检测**（所有 commit 后）：用 `git diff --name-only {checkpoint_sha} HEAD` 检查子任务执行期间所有 commit 的变更（CLI 可能自行 commit 多次），命中则 `git reset --hard {checkpoint_sha}` 回滚到子任务执行前状态

**敏感文件模式**（可配置，fnmatch 匹配文件 basename）:
```yaml
...
> **注意**：匹配目标是文件 basename（不含目录路径），避免 `src/token/` 等正常目录误触发。

**验收标准**（L1 + L2 混合）:
- [ ] Smart Checkpoint 中 `git add -A` 后、`commit` 前，用 `git diff --cached --name-only` 检查暂存区 — **L2: mock git diff 返回文件列表**
- [ ] 所有 commit 完成后运行 `git diff --name-only {checkpoint_sha} HEAD` 获取全量变更 — **L2: mock git diff**
- [ ] 变更文件 basename 与 `sensitive_patterns` 匹配时自动 `git reset --hard {checkpoint_sha}` 回滚 — **L2: mock git reset，验证调用**
...
3. `claude_code_cli.py` 创建子进程时必须加 `start_new_session=True`（创建新进程组，否则 `os.killpg` 会 kill 平台自身进程）
4. 超时 kill 策略：SIGTERM → 等待 5 秒 → SIGKILL 降级，`try/except ProcessLookupError` 防止进程已退出时报错
